...
Air3 AI 运维眼镜用于让现场小白把眼前场景拍给主 AI，并获得中文现场反馈与安全操作指导。服务器 SSH 恢复是当前默认运维模板之一；当画面确实是服务器控制台、终端、登录界面或命令输出时，主 AI 才继续给诊断/恢复/复测指导。

## 角色边界

- Air3 相机是眼睛：负责拍摄需要 AI 判断的现场画面；服务器本地控制台、登录界面、黑底终端或命令输出只是其中一类场景。
...
- 全屏相机预览背景。
- 顶部：`叮当X AI 运维眼镜 / AI 运维现场指导`。
- 中部：绿色取景框和主 AI 指导。
- 底部：短状态与操作提示。
- 所有可见内容必须是现场小白能理解的中文业务文案。
- 页面状态必须覆盖相机就绪、拍照上传、语音录入、语音转写、AI 综合分析、现场反馈、操作指令、照片不清晰、信息不足、语音不清楚、网络错误、远程复测、完成、建议转人工。
...
- `feedbackCode`：识别问题或异常原因，例如照片不清晰、信息不足、语音不清楚、图片和语音冲突。
...
- 文档明确小白最终决定是否人工介入。
- 文档明确清晰照片必须得到 AI 真实反馈，不能因为不是服务器控制台就直接判错。
...
- 首页和采集提示必须使用通用现场画面文案，不把产品限定为服务器控制台。
...
- 通用反馈测试：任意清晰现场画面位于绿色取景框内，主 AI 必须先反馈它看到的内容，并可结合语音说明追问或建议下一步。
- 服务器模板测试：物理屏幕前景为真正服务器控制台或终端窗口，文字位于绿色取景框内，至少完成一轮：拍控制台 -> 主 AI 返回诊断命令 -> 拍诊断输出 -> 主 AI 返回恢复命令。
- 若现场画面不合格，眼镜必须显示“场景不匹配/照片不清晰/信息不足”等中文反馈，而不是崩溃或显示调试内容。
