param(
  ...
  [int]$WaitSeconds = 120,
  [string]$EvidencePrefix = "tmp\air3-instant-chat-501"
)
...
if ($badging -notmatch [regex]::Escape("package: name='$Package'")) {
  throw "instant chat package id mismatch in APK badging"
}
if ($badging -notmatch "versionCode='$ExpectedVersionCode'") {
  throw "instant chat versionCode mismatch in APK badging"
}
if ($badging -notmatch [regex]::Escape("versionName='$ExpectedVersionName'")) {
  throw "instant chat versionName mismatch in APK badging"
}
if ($badging -notmatch [regex]::Escape("application-label:'$ExpectedLabel'")) {
  throw "instant chat app label mismatch in APK badging"
}
...
if ($dump -notmatch "versionCode=$ExpectedVersionCode") {
  throw "Instant chat package versionCode mismatch; expected $ExpectedVersionCode"
}
if ($dump -notmatch "versionName=$([regex]::Escape($ExpectedVersionName))") {
  throw "Instant chat package versionName mismatch; expected $ExpectedVersionName"
}
...
if ($resolved -notmatch [regex]::Escape("$Package/$Activity")) {
  throw "Instant chat package activity did not resolve: $resolved"
}
...
$conversationMarker = -join @([char]0x6700, [char]0x65B0, [char]0x5BF9, [char]0x8BDD)
$evidenceMarker = -join @([char]0x73B0, [char]0x573A, [char]0x8BC1, [char]0x636E)
$captureMarker = -join @([char]0x62CD, [char]0x7167, [char]0x66F4, [char]0x65B0, [char]0x753B, [char]0x9762)
$voiceMarker = -join @([char]0x8BED, [char]0x97F3, [char]0x63D0, [char]0x95EE)
$backMarker = -join @([char]0x8FD4, [char]0x56DE, " / ", [char]0x91CD, [char]0x62CD)
foreach ($marker in @($conversationMarker, $evidenceMarker, $captureMarker, $voiceMarker, $backMarker)) {
