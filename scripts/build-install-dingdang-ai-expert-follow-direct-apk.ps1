...
$buildScript = Join-Path $repoRoot "scripts\build-dingdang-ai-expert-follow-apk.ps1"
$installScript = Join-Path $repoRoot "scripts\install-and-verify-dingdang-ai-expert-follow.ps1"
...
function Read-SecretOrEnv([string]$Name, [string]$PathValue, [switch]$Required) {
  ...
  if (Test-Path -LiteralPath $PathValue) {
    ...
    if ($fileValue.Length -gt 0) { return $fileValue }
  }
  if ($Required) { throw "$Name missing. Set env:$Name or create $PathValue" }
  return ""
}
...
try {
  ...
  Write-Output "Building direct AI expert follow APK. Key values are loaded but will not be printed."
