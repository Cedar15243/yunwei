...
const WorkflowStudioPage = lazy(async () => {
...
export function App({ initialAuthenticated = false, api, auth }: { initialAuthenticated?: boolean; api?: ManagementApi; auth?: WebAuth }) {
  const [authenticated, setAuthenticated] = useState(initialAuthenticated); const [section, setSection] = useState<Section>("工作台"); const [taskId, setTaskId] = useState(""); const [workflowId, setWorkflowId] = useState(""); const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [loginError, setLoginError] = useState(""); const [signingIn, setSigningIn] = useState(false);
  ...
  return <div className="shell"><aside className="nav"><div className="nav-brand"><span className="brand-mark"><MonitorSmartphone size={18} /> D</span><strong>叮当运维</strong></div><nav aria-label="主导航">{navigation.map(({ label, icon: Icon }) => <a aria-current={section === label ? "page" : undefined} key={label} href={`#${label}`} className={section === label ? "active" : ""} onClick={(event) => { event.preventDefault(); setSection(label); setTaskId(""); setWorkflowId(""); }}><Icon size={18} />{label}</a>)}</nav><div className="nav-foot"><span className="presence" />云端管理服务</div></aside><main className="workspace"><header className="command-bar"><div><span className="eyebrow">现场运维工作台</span><h1>{workflowId ? "工作流编排" : taskId ? "任务时间线" : section}</h1></div><div className="operator"><span className="presence" />已授权访问</div></header><section className={workflowId ? "content workflow-content" : "content"}>{api ? workflowId ? <Suspense fallback={<section className="notice workflow-loading">正在打开工作流编排…</section>}><WorkflowStudioPage api={api} workflowId={workflowId} onBack={() => setWorkflowId("")} /></Suspense> : taskId ? <TaskTimelinePage taskId={taskId} api={api} onBack={() => setTaskId("")} /> : <Page section={section} api={api} onOpenTask={setTaskId} onOpenWorkflow={setWorkflowId} /> : <section className="notice error">管理服务尚未完成授权配置，无法读取任务数据。</section>}</section></main></div>;
}

function Page({ section, api, onOpenTask, onOpenWorkflow }: { section: Section; api: ManagementApi; onOpenTask: (taskId: string) => void; onOpenWorkflow: (workflowId: string) => void }) { if (section === "工作台") return <DashboardPage api={api} onOpenTask={onOpenTask} />; if (section === "项目与任务") return <TaskListPage api={api} onOpenTask={onOpenTask} />; if (section === "现场应用") return <FieldAppsPage api={api} onOpenWorkflow={onOpenWorkflow} />; if (section === "媒体中心") return <MediaCenterPage api={api} />; return <DevicesPage api={api} />; }
